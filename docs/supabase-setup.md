...
この文書は、NMF2026-2のクラウド保存に必要なSupabaseデータベース設定をまとめたものです。画面側ではSupabase Authでログインした利用者だけがデータを操作します。閲覧共有リンクを開く利用者にもログインが必要です。
...
すべてのRPCはSupabase Authでログイン済みの`authenticated`利用者だけが実行できます。未ログインの`anon`および`PUBLIC`には実行権限がありません。
...
- RPCの実行権限は`PUBLIC`と`anon`から剥奪し、`authenticated`だけへ付与します。
...
このURLを再度開くと、ログイン後に`localStorage`の内容ではなくSupabase上の最新版を読み込みます。共有URLの`?share=<token>`も同じく、起動時に共有RPCから最新版を読み込みます。

画面を開いたままの場合は15秒ごと、およびタブへ戻った時に最新版を確認します。ただし、次の場合は利用者の作業を守るため自動反映しません。
...
カードの入力内容はモーダル内の下書きです。「編集を完了」を押した時だけ本体へ反映され、その後にローカル保存とクラウド自動保存が動きます。「閉じる」、Esc、背景クリックでは下書きを破棄します。カード選択と表示タブは端末内だけに保存し、クラウドの版数には含めません。
